...
public class PacienteService {
    ...
    /**
     * Obtém o PacienteModel pelo ID.
     * Este método é para uso interno por outros serviços que precisam da entidade completa.
     *
     * @param id ID do paciente.
     * @return PacienteModel representando o paciente encontrado.
     * @throws ObjectNotFoundException Se o paciente não for encontrado.
     */
    @Transactional(readOnly = true)
    public PacienteModel obterPacienteModelPorId(Integer id) {
        return pacienteRepository.findById(id)
                .orElseThrow(() -> new ObjectNotFoundException("Paciente com ID " + id + " não encontrado."));
    }

    @Transactional(readOnly = true)
    public PacienteModel obterPacienteModelPorIdEAtivo(Integer id) {
        return pacienteRepository.findByIdAndAtivoTrue(id)
                .orElseThrow(() -> new BusinessRuleException("Paciente com ID " + id + " não encontrado ou está inativo e não pode ter consultas consultadas."));
    }
     ...
     *
     * @param pacienteExistente pacienteModel contendo os dados atualizados do cliente.
